...
> 适用于 SDK `0.1.3`。目标是让同一个提交可重复通过测试、生成 `.pnp` 并发布。
...
dotnet add .\PclNToolbox.Plugin.Tests package PCLN.Plugin.Testing --version 0.1.3
...
发布前确认项目和 Manifest 都是目标版本，例如 `0.1.3`：

```powershell
git status --short
git tag -s v0.1.3 -m "PCL N Toolbox 0.1.3"
