...
```xml
<PackageReference Include="PCLN.Plugin.UI" Version="0.2.0" />
```

需要创建完整 Avalonia 页面、窗口或申请 Raw Avalonia访问时，再加入：

```xml
<PackageReference Include="PCLN.Plugin.UI.Avalonia" Version="0.2.0" />
```
...
```xml
<PclNPluginSdkVersion>0.2.0</PclNPluginSdkVersion>
```
