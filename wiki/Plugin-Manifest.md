...
`fingerprint` 是主签名指纹，`fingerprints` 可声明额外有效签名 key，用于团队多签或密钥轮换。`revokedFingerprints` 列出已撤销、不得再用于新包签名的完整指纹。`signingPolicy.minimumValidSignatures` 指定包内必须通过验证的签名数量，不能超过当前 active key 数量。

最简单的单签插件只需要 `signing.fingerprint`：
...
必须使用完整 OpenPGP 指纹，不能写短 Key ID。正式 `.pnp` 的签名、公钥与 Manifest 中的 active 指纹集合必须一致。
