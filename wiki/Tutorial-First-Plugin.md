...
> 适用于 SDK `0.2.2`。完成后你会得到一个能在 PCL N 中安装、注册命令并显示通知的 `.pnp`。
...
```text
PclNToolbox.Plugin/bin/Release/net10.0/dev.example.toolbox-0.2.2.pnp
```

`.pnp` 是 ZIP 容器，可以只读检查：

```powershell
tar -tf .\PclNToolbox.Plugin\bin\Release\net10.0\dev.example.toolbox-0.2.2.pnp
```
